    ...
    def interact_action(self, state, target):
        ...
        if pos != stand:
            ...
            if a is not None:
                return a
        if pos == stand and ori != face:
            # Overcooked direction actions also rotate the player when the
            # adjacent cell is a feature.  This makes edge facilities usable
            # even when there is no second floor cell behind the stand.
            return face
