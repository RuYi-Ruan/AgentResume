    ...
    for p in grid.pot_locs:
        if max(abs(p[0] - px), abs(p[1] - py)) <= 1:
            if state.has_object(p):
                ...
                if soup.is_ready:
                    seen[str(p)] = {"kind": "ready", "t": state.timestep, "source": "visible"}
                elif soup.is_cooking:
                    seen[str(p)] = {"kind": "cooking", "t": state.timestep, "source": "visible"}
                else:
                    seen[str(p)] = {
                        "kind": f"items{len(soup.ingredients)}",
                        "t": state.timestep,
                        "source": "visible",
                    }
            else:
                seen[str(p)] = {"kind": "empty", "t": state.timestep, "source": "visible"}
    return {
        "me": me,
