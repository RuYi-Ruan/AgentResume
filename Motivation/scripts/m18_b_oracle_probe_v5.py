"""Development-only oracle probe for candidate PublicBobV5."""
...
def main():
    ...
    model.eval()
    destination = source / "B_controller_probe_v5"
    if destination.exists():
        raise RuntimeError("B controller v5 probe already exists; do not overwrite")
    ...
    for seed in episode_seeds(config, args.group, "development"):
        ...
        emit(root / "progress.jsonl", {"stage": "B_controller_probe_v5", "group": args.group,
    ...
    report = {"controller": PublicBobV5.version, "group": args.group, "starts": len(paired),
              ...
              "source_hashes": {p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
                                for p in ("src/ocres/m18_b.py", "src/ocres/m18_bob_v5.py", __file__)}}
