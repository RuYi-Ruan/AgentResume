"""Recover five group-8 episodes affected by the 18:40--19:30 outage."""
...
NEW_LIMIT = 29000
SELECTED_NAMES = {
    "906015_updated", "906006_none", "906003_none",
    "906010_updated", "906000_updated",
}
...
def main() -> None:
    ...
    for row in read_jsonl(original_responses):
        ...
        if not errors.intersection({"URLError", "TimeoutError", "ConnectionError",
                                    "transport_exhausted"}):
            continue
    ...
    if not SELECTED_NAMES <= failures.keys():
        raise RuntimeError(f"recovery set is not backed by failures: {failures}")
    ...
    manifest = {
        **original_manifest,
        "recovery": TAG,
        "reason": "five episodes affected by the 18:40--19:30 network outage",
    ...
    transport_healthcheck(probe_messages, options,
                          ...
                          root / "api_budget.jsonl", "B_group8_anomaly_recovery2")
