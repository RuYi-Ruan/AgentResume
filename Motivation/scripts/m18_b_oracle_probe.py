"""Development-only B controller utility probe; no Qwen calls or formal starts."""
...
def main():
    ...
    model.eval()
    destination = source / "B_controller_probe"
    if destination.exists():
        raise RuntimeError("B controller probe already exists; do not overwrite")
    ...
    for seed in episode_seeds(config, args.group, "development"):
        none = run_b_episode(group["rows"], seed, model, spec, config, "none_local_unknown", unknown)
        ...
        paired.append({"seed": seed, "none_300": none["score_first_300"],
                       ...
                       "none_700": none["score_full_700"],
                       "oracle_700": oracle["score_full_700"],
                       "none_queries": none["query_count"],
                       "oracle_queries": oracle["query_count"]})
        emit(root / "progress.jsonl", {"stage": "B_controller_probe", "group": args.group,
                                       "seed": seed, "none_300": none["score_first_300"],
                                       "oracle_300": oracle["score_first_300"]})
    differences = [row["oracle_300"]-row["none_300"] for row in paired]
    report = {"group": args.group, "starts": len(paired), "paired": paired,
              "mean_300_gain_soups": sum(differences)/len(differences)/20,
              ...
              "source_hashes": {p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
                                for p in ("src/ocres/m18_b.py", "src/ocres/m18_bob.py", __file__)}}
