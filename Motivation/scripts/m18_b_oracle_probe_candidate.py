"""Development-only oracle probe for a named candidate Bob controller."""
...
from ocres.m18_bob_v4 import PublicBobV4
...
def main():
    ...
    model.eval()
    destination = source / "B_controller_probe_v4"
    if destination.exists():
        raise RuntimeError("B controller v4 probe already exists; do not overwrite")
    ...
    for seed in episode_seeds(config, args.group, "development"):
        ...
        paired.append({"seed": seed, "none_300": none["score_first_300"],
                       ...
                       "oracle_queries": oracle["query_count"], "none_blocks": none["bob_block_events"],
                       "oracle_blocks": oracle["bob_block_events"]})
        emit(root / "progress.jsonl", {"stage": "B_controller_probe_v4", "group": args.group,
    ...
    report = {"controller": PublicBobV4.version, "group": args.group, "starts": len(paired),
              ...
              "mean_block_gain": sum(row["none_blocks"] - row["oracle_blocks"] for row in paired) / len(paired),
              "source_hashes": {p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
                                for p in ("src/ocres/m18_b.py", "src/ocres/m18_bob_v2.py",
                                          "src/ocres/m18_bob_v4.py", __file__)}}
